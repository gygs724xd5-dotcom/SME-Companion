...
def _missing_information(application_state: dict, capability_key: str, workflow: str | None) -> list[str]:
    ...
    if workflow in REQUIRED_FIELDS:
        ...
        if workflow == WORKFLOW_SALES_PLAN_7_DAY:
            ...
            if not (collected.get("ingredients_costs") or collected.get("total_cost") or collected.get("cost") or collected.get("unit_cost") or collected.get("cost_per_unit")):
...
def _looks_like_workflow_answer(message: str) -> bool:
    ...
    if any(token in lowered for token in question_words):
        return False
